...
@router.message(F.text == "🎯 Изменить цель")  # временно, для плавного перехода к новой системе
...
async def cmd_start(message: Message):
    ...
    await message.answer(
        f"👋 Привет, {message.from_user.first_name}! Бот активирована!",
...
async def show_league_info(callback: CallbackQuery):
    ...
    next_info = f"\n\n🔜 До следующей лиги осталось баллов: *{league['next'] - user['score']}*"\
        if league["next"] <= 100 else "\n\n🌟 Вы на вершине!"
